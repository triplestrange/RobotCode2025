 ...
 */
public class InterpolatingTreeMapBuffer<K extends InverseInterpolable<K> & Comparable<K>, V extends Interpolatable<V>>
        extends TreeMap<K, V> {
    private static final long serialVersionUID = 8347275262778054124L;

    final int max_;

    public InterpolatingTreeMapBuffer(int maximumSize) {
        max_ = maximumSize;
    }

    public InterpolatingTreeMapBuffer() {
        this(0);
    }
