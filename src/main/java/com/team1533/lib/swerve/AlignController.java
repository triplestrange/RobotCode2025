...
public class AlignController {
  ...
  public ChassisSpeeds update(ChassisSpeeds input) {
    ...
    boolean override =
        !RobotContainer.getInstance().isLeft() || !RobotContainer.getInstance().isRight();

    // if (current.getTranslation().getDistance(target.getTranslation()) < -1000 || override)   {
    if (override) {
      skewed.vxMetersPerSecond =
          MathUtil.clamp(
              pidTranslationController.calculate(current.getX(), target.getX()), -1., 1.);
      skewed.vyMetersPerSecond =
          MathUtil.clamp(
              pidTranslationController.calculate(current.getY(), target.getY()), -1., 1.);
    }
