...
public class IntakeIOReal implements IntakeIO {
    ...
    public IntakeIOReal() {
        leaderTalon = new TalonFX(IntakeConstants.intakeTalon, IntakeConstants.canBUS);
        wristEncoder = new CANcoder(IntakeConstants.wristEncoderCanID, IntakeConstants.canBUS);

        // Leader motor configs
        config.Slot0.kP = IntakeConstants.gains.kP();
        config.Slot0.kI = IntakeConstants.gains.kI();
        config.Slot0.kD = IntakeConstants.gains.kD();
        config.Slot0.kA = IntakeConstants.gains.ffkA();
        config.Slot0.kG = IntakeConstants.gains.ffkG();
        config.Slot0.kS = IntakeConstants.gains.ffkS();
        config.Slot0.kV = IntakeConstants.gains.ffkV();

        config.TorqueCurrent.PeakForwardTorqueCurrent = IntakeConstants.torqueCurrentLimit;
        config.TorqueCurrent.PeakReverseTorqueCurrent = -IntakeConstants.torqueCurrentLimit;
        config.MotorOutput.Inverted = IntakeConstants.leaderInverted
                ? InvertedValue.Clockwise_Positive
                : InvertedValue.CounterClockwise_Positive;
        config.MotorOutput.NeutralMode = NeutralModeValue.Brake;
        config.Feedback.SensorToMechanismRatio = IntakeConstants.reduction;

        config.Feedback.FeedbackRemoteSensorID = wristEncoder.getDeviceID();
        config.Feedback.FeedbackSensorSource = FeedbackSensorSourceValue.FusedCANcoder;
        config.Feedback.SensorToMechanismRatio = IntakeConstants.SensorToMechanismRatio;
        config.Feedback.RotorToSensorRatio = IntakeConstants.reduction;
        ...
        config.CurrentLimits.SupplyCurrentLowerLimit = IntakeConstants.supplyCurrentLowerLimit;
        config.CurrentLimits.SupplyCurrentLowerTime = IntakeConstants.supplyCurrentLowerLimitTime;

        config.MotionMagic.MotionMagicCruiseVelocity = IntakeConstants.motionMagicCruiseVelocity;
        config.MotionMagic.MotionMagicAcceleration = IntakeConstants.motionMagicAcceleration;
        config.MotionMagic.MotionMagicJerk = IntakeConstants.motionMagicJerk;

        config.SoftwareLimitSwitch.ForwardSoftLimitEnable = false;
        config.SoftwareLimitSwitch.ForwardSoftLimitThreshold = IntakeConstants.forwardSoftLimitThreshold;
        config.SoftwareLimitSwitch.ReverseSoftLimitEnable = false;
        config.SoftwareLimitSwitch.ReverseSoftLimitThreshold = IntakeConstants.reverseSoftLimitThreshold;

        // Cancoder configs
        encoderConfig.MagnetSensor.AbsoluteSensorDiscontinuityPoint = IntakeConstants.absEncoderDiscontinuity;
        encoderConfig.MagnetSensor.MagnetOffset = IntakeConstants.absEncoderOffset;
